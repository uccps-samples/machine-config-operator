...
import (
	...
	apicfgv1 "github.com/uccps-samples/api/config/v1"
	apioperatorsv1alpha1 "github.com/uccps-samples/api/operator/v1alpha1"
	fakeconfigv1client "github.com/uccps-samples/client-go/config/clientset/versioned/fake"
	configv1informer "github.com/uccps-samples/client-go/config/informers/externalversions"
	fakeoperatorclient "github.com/uccps-samples/client-go/operator/clientset/versioned/fake"
...
func (f *fixture) expectPatchContainerRuntimeConfig(config *mcfgv1.ContainerRuntimeConfig, patch []byte) {
	f.actions = append(f.actions, core.NewRootPatchAction(schema.GroupVersionResource{Version: "v1", Group: "machineconfiguration.uccp.io", Resource: "containerruntimeconfigs"}, config.Name, types.MergePatchType, patch))
}

func (f *fixture) expectUpdateContainerRuntimeConfig(config *mcfgv1.ContainerRuntimeConfig) {
	f.actions = append(f.actions, core.NewRootUpdateSubresourceAction(schema.GroupVersionResource{Version: "v1", Group: "machineconfiguration.uccp.io", Resource: "containerruntimeconfigs"}, "status", config))
}
...
func TestContainerRuntimeConfigCreate(t *testing.T) {
	for _, platform := range []apicfgv1.PlatformType{apicfgv1.AWSPlatformType, apicfgv1.NonePlatformType, "unrecognized"} {
		t.Run(string(platform), func(t *testing.T) {
			...
			ctrcfg1 := newContainerRuntimeConfig("set-log-level", &mcfgv1.ContainerRuntimeConfiguration{LogLevel: "debug", LogSizeMax: resource.MustParse("9k"), OverlaySize: resource.MustParse("3G")}, metav1.AddLabelToSelector(&metav1.LabelSelector{}, "pools.operator.machineconfiguration.uccp.io/master", ""))
