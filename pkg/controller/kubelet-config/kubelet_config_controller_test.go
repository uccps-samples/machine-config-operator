...
import (
	...
	osev1 "github.com/uccps-samples/api/config/v1"
	oseconfigfake "github.com/uccps-samples/client-go/config/clientset/versioned/fake"
	oseinformersv1 "github.com/uccps-samples/client-go/config/informers/externalversions"
	...
	ctrlcommon "github.com/uccps-samples/machine-config-operator/pkg/controller/common"
	"github.com/uccps-samples/machine-config-operator/pkg/generated/clientset/versioned/fake"
	informers "github.com/uccps-samples/machine-config-operator/pkg/generated/informers/externalversions"
	"github.com/uccps-samples/machine-config-operator/pkg/version"
	"github.com/uccps-samples/machine-config-operator/test/helpers"
)
...
func (f *fixture) expectGetKubeletConfigAction(config *mcfgv1.KubeletConfig) {
	f.actions = append(f.actions, core.NewRootGetAction(schema.GroupVersionResource{Version: "v1", Group: "machineconfiguration.uccp.io", Resource: "kubeletconfigs"}, config.Name))
}

func (f *fixture) expectGetMachineConfigAction(config *mcfgv1.MachineConfig) {
	f.actions = append(f.actions, core.NewRootGetAction(schema.GroupVersionResource{Version: "v1", Group: "machineconfiguration.uccp.io", Resource: "machineconfigs"}, config.Name))
}

func (f *fixture) expectCreateMachineConfigAction(config *mcfgv1.MachineConfig) {
	f.actions = append(f.actions, core.NewRootCreateAction(schema.GroupVersionResource{Version: "v1", Group: "machineconfiguration.uccp.io", Resource: "machineconfigs"}, config))
}

func (f *fixture) expectUpdateMachineConfigAction(config *mcfgv1.MachineConfig) {
	f.actions = append(f.actions, core.NewRootUpdateAction(schema.GroupVersionResource{Version: "v1", Group: "machineconfiguration.uccp.io", Resource: "machineconfigs"}, config))
}

func (f *fixture) expectPatchKubeletConfig(config *mcfgv1.KubeletConfig, patch []byte) {
	f.actions = append(f.actions, core.NewRootPatchAction(schema.GroupVersionResource{Version: "v1", Group: "machineconfiguration.uccp.io", Resource: "kubeletconfigs"}, config.Name, types.MergePatchType, patch))
}

func (f *fixture) expectUpdateKubeletConfig(config *mcfgv1.KubeletConfig) {
	f.actions = append(f.actions, core.NewRootUpdateSubresourceAction(schema.GroupVersionResource{Version: "v1", Group: "machineconfiguration.uccp.io", Resource: "kubeletconfigs"}, "status", config))
}

func (f *fixture) expectUpdateKubeletConfigRoot(config *mcfgv1.KubeletConfig) {
	f.actions = append(f.actions, core.NewRootUpdateAction(schema.GroupVersionResource{Version: "v1", Group: "machineconfiguration.uccp.io", Resource: "kubeletconfigs"}, config))
}
...
func TestKubeletConfigCreate(t *testing.T) {
	for _, platform := range []osev1.PlatformType{osev1.AWSPlatformType, osev1.NonePlatformType, "unrecognized"} {
		t.Run(string(platform), func(t *testing.T) {
			...
			kc1 := newKubeletConfig("smaller-max-pods", &kubeletconfigv1beta1.KubeletConfiguration{MaxPods: 100}, metav1.AddLabelToSelector(&metav1.LabelSelector{}, "pools.operator.machineconfiguration.uccp.io/master", ""))
	...
	for _, platform := range []osev1.PlatformType{osev1.AWSPlatformType, osev1.NonePlatformType, "unrecognized"} {
		t.Run(string(platform), func(t *testing.T) {
			...
			for i := 0; i < kcCount; i++ {
				...
				poolLabelName := fmt.Sprintf("pools.operator.machineconfiguration.uccp.io/%s", poolName)
		...
		t.Run(string(platform), func(t *testing.T) {
			...
			kc1 := &mcfgv1.KubeletConfig{
				...
				Spec: mcfgv1.KubeletConfigSpec{
					...
					MachineConfigPoolSelector: metav1.AddLabelToSelector(&metav1.LabelSelector{}, "pools.operator.machineconfiguration.uccp.io/master", ""),
				},
				Status: mcfgv1.KubeletConfigStatus{},
			}
			kubeletConfigKey, _ := getManagedKubeletConfigKey(mcp, f.client, kc1)
