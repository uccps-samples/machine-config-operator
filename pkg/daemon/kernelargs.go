...
import (
	...
	"github.com/uccps-samples/machine-config-operator/pkg/daemon/pivot/types"
...
// tuneableUSwiftArgsAllowlist contains allowed keys for tunable kernel arguments on UOS
var tuneableUSwiftArgsAllowlist = map[string]bool{
	"nosmt": true,
}
...
func isArgTunable(arg string) (bool, error) {
	...
	if os.IsUSwift() {
		return tuneableUSwiftArgsAllowlist[arg], nil
	} else if os.IsRHCOS() {
