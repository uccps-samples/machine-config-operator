...
metadata:
  ...
  name: controllerconfigs.machineconfiguration.uccp.io
  labels:
    "uccp.io/operator-managed": ""
spec:
                    ...
                    properties:
                      baseDomain:
                        description: "baseDomain is the base domain of the cluster.
                          ...
                          \n For example, given the base domain ` + "`" + `uccp.example.com` + "`" + `,
                          an API server DNS record may be created for ` + "`" + `cluster-api.uccp.example.com` + "`" + `.
                    ...
                    properties:
                      cloudConfig:
                        description: "cloudConfig is a reference to a ConfigMap containing
                          ...
                          for this config map is uccp-config. \n cloudConfig
                          ...
                          ` + "`" + `kube-cloud-config` + "`" + ` in ` + "`" + `uccp-config-managed` + "`" + ` namespace
                            ...
                            properties:
                              ...
                              resourceTags:
                                description: resourceTags is a list of additional
                                  ...
                                  a maximum of 50 tags per resource. uccp reserves
...
- apiGroups: ["machineconfiguration.uccp.io"]
  resources: ["*"]
  verbs: ["*"]
- apiGroups: [""]
  resources: ["configmaps", "secrets"]
  verbs: ["*"]
- apiGroups: ["config.uccp.io"]
  resources: ["images", "clusterversions", "featuregates"]
  verbs: ["*"]
- apiGroups: ["config.uccp.io"]
  resources: ["schedulers", "apiservers"]
  verbs: ["get", "list", "watch"]
- apiGroups: ["operator.uccp.io"]
...
var _manifestsMachineconfigcontrollerControllerconfigYaml = []byte(`apiVersion: machineconfiguration.uccp.io/v1
kind: ControllerConfig
metadata:
  name: machine-config-controller
  annotations:
    machineconfiguration.uccp.io/generated-by-version: "{{ .Version }}"
spec:
  ...
  template:
    ...
    spec:
      ...
      - name: oauth-proxy
        ...
        - --https-address=:9001
        - --provider=uccp
        - --uccp-service-account=machine-config-daemon
        - --upstream=http://127.0.0.1:8797
        - --tls-cert=/etc/tls/private/tls.crt
        - --tls-key=/etc/tls/private/tls.key
        - --cookie-secret-file=/etc/tls/cookie-secret/cookie-secret
        - '--uccp-sar={"resource": "namespaces", "verb": "get"}'
        - '--uccp-delegate-urls={"/": {"resource": "namespaces", "verb": "get"}}'
        resources:
          requests:
            cpu: 20m
            memory: 50Mi
...
var _manifestsMachineconfigserverCsrBootstrapRoleBindingYaml = []byte(`# system-bootstrap-node-bootstrapper lets serviceaccount ` + "`" + `uccp-machine-config-operator/node-bootstrapper` + "`" + ` tokens and nodes request CSRs.
apiVersion: rbac.authorization.k8s.io/v1
kind: ClusterRoleBinding
metadata:
...
- kind: ServiceAccount
  ...
  namespace: uccp-machine-config-operator
roleRef:
  kind: ClusterRole
...
var _manifestsMasterMachineconfigpoolYaml = []byte(`apiVersion: machineconfiguration.uccp.io/v1
kind: MachineConfigPool
metadata:
  name: master
  labels:
    "operator.machineconfiguration.uccp.io/required-for-upgrade": ""
    "machineconfiguration.uccp.io/mco-built-in": ""
    "pools.operator.machineconfiguration.uccp.io/master": ""
spec:
...
metadata:
  name: coredns
  namespace: uccp-{{ onPremPlatformShortName .ControllerConfig }}-infra
  creationTimestamp:
...
spec:
  ...
  - name: render-config
    ...
    - "/etc/coredns"
    - "--cluster-config"
    - "/opt/uccp/manifests/cluster-config.yaml"
    resources: {}
    volumeMounts:
    - name: kubeconfig
      mountPath: "/etc/kubernetes/kubeconfig"
...
var _manifestsWorkerMachineconfigpoolYaml = []byte(`apiVersion: machineconfiguration.uccp.io/v1
kind: MachineConfigPool
metadata:
  name: worker
  labels:
