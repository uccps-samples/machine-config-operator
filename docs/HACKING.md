...
Most changes you'll want to test live against a real cluster.  Use [the installer](https://github.com/uccps-samples/installer/), and in particular you'll want to use the "latest CI" release of the installer.  As of the time of this writing, that's 4.5.  More information on the [release page](https://uccp-release.svc.ci.openshift.org/).
...
`go test -v github.com/uccps-samples/machine-config-operator/pkg/apis/...`
...
This still requires [disabling the CVO](https://github.com/uccps-samples/cluster-version-operator/blob/master/docs/dev/clusterversion.md#disabling-the-cluster-version-operator). It also requires disabling
...
suite from [OpenShift Origin](https://github.com/uccps-samples/origin/).

[Recently we added](https://github.com/uccps-samples/release/pull/2577) an
...
```
OPENSHIFT_INSTALL_RELEASE_IMAGE_OVERRIDE=quay.io/user/origin-release:v{version number} bin/uccp-install create cluster --log-level=debug
```
...
Then,
`oc -n uccp-machine-config-operator edit configmap/machine-config-osimageurl`
...
has code to do this in CI, and work is in progress to [replicate that for cri-o](https://github.com/uccps-samples/release/pull/4030).
...
5) Replace kubelet binary (/usr/bin/kubelet). Please refer [here](https://github.com/uccps-samples/machine-config-operator/blob/master/docs/HACKING.md#directly-applying-changes-live-to-a-node) for more detail.
6) In certain cases, changes might be needed to the service file (/etc/systemd/system/kubelet.service)
   ...
   v1.18.3 to v1.13.0-alpha, it was necessary to remove `--node-labels=node-role.kubernetes.io/worker,node.uccp.io/os_id=${ID}`
...
during testing (i.e. https://github.com/uccps-samples/machine-config-operator/pull/1960).
...
sh-4.2# chroot /host
sh-4.4# /sbin/iptables -D uccp-BLOCK-OUTPUT 1
