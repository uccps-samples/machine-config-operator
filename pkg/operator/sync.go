...
import (
	...
	mcoResourceApply "github.com/uccps-samples/machine-config-operator/lib/resourceapply"
	mcoResourceRead "github.com/uccps-samples/machine-config-operator/lib/resourceread"
	"github.com/uccps-samples/machine-config-operator/manifests"
	...
	templatectrl "github.com/uccps-samples/machine-config-operator/pkg/controller/template"
	daemonconsts "github.com/uccps-samples/machine-config-operator/pkg/daemon/constants"
	"github.com/uccps-samples/machine-config-operator/pkg/server"
	"github.com/uccps-samples/machine-config-operator/pkg/version"
)

const (
	requiredForUpgradeMachineConfigPoolLabelKey = "operator.machineconfiguration.uccp.io/required-for-upgrade"
)
...
// Sync cloud config on supported platform from cloud.conf available in uccp-config-managed/kube-cloud-config ConfigMap.
func (optr *Operator) syncCloudConfig(spec *mcfgv1.ControllerConfigSpec, infra *configv1.Infrastructure) error {
	cm, err := optr.clusterCmLister.ConfigMaps("uccp-config-managed").Get("kube-cloud-config")
	if err != nil {
		if apierrors.IsNotFound(err) {
			if isKubeCloudConfigCMRequired(infra) {
				...
				return fmt.Errorf("%s/%s configmap is required on platform %s but not found: %v",
					"uccp-config-managed", "kube-cloud-config", infra.Status.PlatformStatus.Type, err)
			}
			return nil
		}
		return err
	}
	// Read cloud.conf from uccp-config-managed/kube-cloud-config ConfigMap.
	...
	if err != nil {
		if isCloudConfRequired(infra) {
			...
			return fmt.Errorf("%s/%s configmap must have the %s key on platform %s but not found",
				"uccp-config-managed", "kube-cloud-config", "cloud.conf", infra.Status.PlatformStatus.Type)
		}
	} else {
...
func (optr *Operator) syncRenderConfig(_ *renderConfig) error {
	...
	initialKubeAPIServerServingCABytes, err := optr.getCAsFromConfigMap("uccp-config", "initial-kube-apiserver-server-ca", "ca-bundle.crt")
	if err != nil {
		return err
	}
	...
	kubeAPIServerServingCABytes, err := optr.getCAsFromConfigMap("uccp-kube-apiserver-operator", "kube-apiserver-to-kubelet-client-ca", "ca-bundle.crt")
	if err != nil {
	...
	additionalTrustBundle, err := optr.getCAsFromConfigMap("uccp-config", "user-ca-bundle", "ca-bundle.crt")
	if err != nil && !apierrors.IsNotFound(err) {
		return err
	}
	if len(additionalTrustBundle) > 0 {
		if !certPool.AppendCertsFromPEM(additionalTrustBundle) {
			return fmt.Errorf("configmap %s/%s doesn't have a valid PEM bundle", "uccp-config", "user-ca-bundle")
		}
	...
	if proxy != nil && proxy.Spec.TrustedCA.Name != "" && proxy.Spec.TrustedCA.Name != "user-ca-bundle" {
		proxyTrustBundle, err := optr.getCAsFromConfigMap("uccp-config", proxy.Spec.TrustedCA.Name, "ca-bundle.crt")
		if err != nil {
			return err
		}
		if len(proxyTrustBundle) > 0 {
			if !certPool.AppendCertsFromPEM(proxyTrustBundle) {
				return fmt.Errorf("configmap %s/%s doesn't have a valid PEM bundle", "uccp-config", proxy.Spec.TrustedCA.Name)
			}
	...
	spec.PullSecret = &corev1.ObjectReference{Namespace: "uccp-config", Name: "pull-secret"}
