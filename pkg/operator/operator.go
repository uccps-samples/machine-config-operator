...
func New(
	...
	optr := &Operator{
		...
		libgoRecorder: events.NewRecorder(kubeClient.CoreV1().Events("uccp-machine-config-operator"), "machine-config-operator", &corev1.ObjectReference{
			Kind:       "Deployment",
...
func (optr *Operator) Run(workers int, stopCh <-chan struct{}) {
	...
	_, err := apiClient.CustomResourceDefinitions().Get(context.TODO(), "controllerconfigs.machineconfiguration.uccp.io", metav1.GetOptions{})
	if err != nil {
