...
import (
	...
	operatorinformers "github.com/uccps-samples/client-go/operator/informers/externalversions"
	...
	mcfginformers "github.com/uccps-samples/machine-config-operator/pkg/generated/informers/externalversions"
...
func CreateControllerContext(cb *clients.Builder, stop <-chan struct{}, targetNamespace string) *ControllerContext {
	...
	openShiftConfigKubeNamespacedSharedInformer := informers.NewFilteredSharedInformerFactory(kubeClient, resyncPeriod()(), "uccp-config", nil)
	openShiftKubeAPIServerKubeNamespacedSharedInformer := informers.NewFilteredSharedInformerFactory(kubeClient,
		resyncPeriod()(),
		"uccp-kube-apiserver-operator",
	...
	kubeMAOSharedInformer := informers.NewFilteredSharedInformerFactory(kubeClient, resyncPeriod()(), "uccp-machine-api", nil)
