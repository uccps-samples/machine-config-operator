...
func waitForConfigDriftMonitorStart(t *testing.T, cs *framework.ClientSet, node corev1.Node) {
	...
	// "machineconfiguration.uccp.io/state" gets set to "Done" before the
	// Config Drift Monitor is started. This is fine, however it makes testing a
	// bit tricky as we rely upon "machineconfiguration.uccp.io/state" to be
...
func pollForResourceState(timeout time.Duration, pollFunc func() (bool, error)) (time.Duration, error) {
	...
	// https://github.com/uccps-samples/machine-config-operator/blob/master/test/e2e-single-node/sno_mcd_test.go#L355-L374
	start := time.Now()
