...
import (
	...
	"github.com/uccps-samples/machine-config-operator/pkg/daemon/constants"
	pivottypes "github.com/uccps-samples/machine-config-operator/pkg/daemon/pivot/types"
	pivotutils "github.com/uccps-samples/machine-config-operator/pkg/daemon/pivot/utils"
)
...
const (
	...
	// SSH Keys for user "uswift" will only be written at /home/uswift/.ssh
	uswiftUserSSHPath = "/home/uswift/.ssh/"
...
func podmanCopy(imgURL, osImageContentDir string) (err error) {
	...
	cidBuf, err = runGetOut("podman", "create", "--net=none", "--annotation=org.uccp.machineconfigoperator.pivot=true", "--name", containerName, imgURL)
	if err != nil {
		return
	}
...
// See also https://github.com/uccps-samples/installer/pull/2594
...
func (dn *Daemon) generateExtensionsArgs(oldConfig, newConfig *mcfgv1.MachineConfig) []string {
	...
	// See https://github.com/uccps-samples/release/blob/959c2954344438c4eed3ec7f52a5e099e8335516/ci-operator/jobs/uccp/release/uccp-release-release-4.7-periodics.yaml#L586
...
func (dn *Daemon) atomicallyWriteSSHKey(keys string) error {
	...
	} else if dn.os.IsUSwift() {
		authKeyPath = filepath.Join(uswiftUserSSHPath, "authorized_keys")
	} else {
...
func (dn *Daemon) getPendingStateLegacyLogger() (*journalMsg, error) {
	...
	cmdLiteral := "journalctl -o cat _UID=0 | grep -v audit | grep uccp_MACHINE_CONFIG_DAEMON_LEGACY_LOG_HACK"
...
type journalMsg struct {
	...
	OldLogger string `json:"uccp_MACHINE_CONFIG_DAEMON_LEGACY_LOG_HACK,omitempty"` // unused today
}
...
func (dn *Daemon) storePendingStateLegacyLogger(pending *mcfgv1.MachineConfig, isPending int) ([]byte, error) {
	...
	oldLogger := exec.Command("logger", fmt.Sprintf(`{"MESSAGE": "%s", "BOOT_ID": "%s", "PENDING": "%d", "uccp_MACHINE_CONFIG_DAEMON_LEGACY_LOG_HACK": "1"}`, pending.GetName(), dn.bootID, isPending))
