...
`,
		want: []manifest{{
			Raw: []byte(`{"apiVersion":"config.uccp.io/v1","kind":"FeatureGate","metadata":{"name":"cluster"},"spec":{"featureSet":"TechPreviewNoUpgrade"}}`),
		}},
	}, {
		name: "two-resources",
		raw: `
...
`,
		want: []manifest{{
			Raw: []byte(`{"apiVersion":"machineconfiguration.uccp.io/v1","kind":"ContainerRuntimeConfig","metadata":{"name":"cr-pid-limit"},"spec":{"containerRuntimeConfig":{"pidsLimit":100000},"machineConfigPoolSelector":{"matchLabels":{"pools.operator.machineconfiguration.uccp.io/master":""}}}}`),
		}},
	}}
