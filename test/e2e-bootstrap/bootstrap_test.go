...
import (
	...
	"github.com/uccps-samples/machine-config-operator/internal/clients"
	mcfgv1 "github.com/uccps-samples/machine-config-operator/pkg/apis/machineconfiguration.uccp.io/v1"
	"github.com/uccps-samples/machine-config-operator/pkg/controller/bootstrap"
	ctrlcommon "github.com/uccps-samples/machine-config-operator/pkg/controller/common"
	containerruntimeconfig "github.com/uccps-samples/machine-config-operator/pkg/controller/container-runtime-config"
	kubeletconfig "github.com/uccps-samples/machine-config-operator/pkg/controller/kubelet-config"
	"github.com/uccps-samples/machine-config-operator/pkg/controller/node"
	"github.com/uccps-samples/machine-config-operator/pkg/controller/render"
	"github.com/uccps-samples/machine-config-operator/pkg/controller/template"
...
const (
	...
	componentNamespace       = "uccp-machine-config-operator"
...
func TestE2EBootstrap(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			manifests: [][]byte{
				[]byte(`apiVersion: config.uccp.io/v1
kind: FeatureGate
metadata:
  name: cluster
spec:
...
spec:
  featureSet: TechPreviewNoUpgrade`),
				[]byte(`apiVersion: machineconfiguration.uccp.io/v1
kind: KubeletConfig
metadata:
...
func checkCleanEnvironment(t *testing.T, clientSet *framework.ClientSet) {
	...
	// ########################################
	// BEGIN: machineconfiguration.uccp.io
	// ########################################
	...
	// ######################################
	// END: machineconfiguration.uccp.io
	// ######################################
